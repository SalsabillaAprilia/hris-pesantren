  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <TodayAgendaCard agendas={agendas} loading={loading} title="Agenda Saya Hari Ini" />
      </div>

      {/* Agenda Direvisi */}
      {revisionAgendas.length > 0 && (
        <Card className="bg-red-50/50 border-red-100 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-red-800 flex items-center gap-2">
              ...
              Agenda Perlu Direvisi ({revisionAgendas.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {revisionAgendas.map((item: any) => (
                <div
                  key={item.id}
                  className="flex items-center justify-between py-2 px-3 rounded-lg bg-white border border-red-100 hover:border-red-200 transition-colors cursor-pointer"
                  onClick={() => navigate("/agenda")}
                >
                  <div>
                    <p className="text-sm font-medium text-slate-800">Laporan Tanggal {item.start_date}</p>
                    <p className="text-[11px] text-red-600 font-medium">Catatan: {item.manager_notes}</p>
                  </div>
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded border whitespace-nowrap text-red-700 bg-red-100 border-red-200">
                    Perlu Revisi
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {recentApprovals.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    onClick={() => navigate("/attendance", { state: { tab: "pengajuan" } })}
